...
public class Triangle {
    ...
    /// <summary>
    /// generates a convex polygon bounded by the points <br></br>
    ...
    public static void drawConvexPolygon((int x, int y)[] points, int color, ref long[,] destination) {
        ...
        for (int y = 0; y < height; y++) {
            for (int? x = edges[y, 0]; x <= edges[y, 1]; x++) {
                destination[(int)x - 1, y + minY] = color;
            }
        }

        for (int i = 0; i < points.Length; i++) {
            destination[points[i].x, points[i].y] = 0x673793;
        }
    }
    
    /// <summary>
    /// generates a polygon bounded by the points <br></br>
    /// the order of the points depends as the lines will be generated from nth to n+1th point 
    /// </summary>
    /// <param name="points">array of points</param>
    /// <param name="color">color in hexadecimal code</param>
    /// <param name="destination">the place where the triangle will be drawn, format MUST be the same as for Global.DISPLAY_COLORS</param>
    /// <seealso cref="Global.DISPLAY_COLORS"/>
    public static void drawPolygon((int x, int y)[] points, int color, ref long[,] destination) {
